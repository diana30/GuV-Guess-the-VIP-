...
if (isset($_POST["actorName"])) {
    ...
    if (strtolower($actorName) ===  strtolower($result["actorName"])) {
        ...
        $conn->query($sql);
        $var = $result['id'] + 1;
        $sql = "INSERT INTO `wrong`(`user_id`, `photo_id`) VALUES ( " . $_SESSION["logat"] . "," . $var . " )";
        $conn->query($sql);
        $q_id = getQuestion($_SESSION["logat"]);
        $sql = "SELECT * FROM `twimage` WHERE id = " . $q_id;
        $result = $conn->query($sql);
        ...
        array_push($error ,"<p class='message'>Wrong !<br> Try again</p>");
        $sql = "UPDATE `twusers` SET score = score - 2 WHERE id = " . $_SESSION["logat"];
        $conn->query($sql);
